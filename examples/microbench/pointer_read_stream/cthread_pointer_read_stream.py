...
# Computation
#-------------------------------------------------------------------------------
ram = CoramInStream(idx=0, datawidth=8*DSIZE*SIMD_WIDTH, size=RAM_SIZE)
channel = CoramChannel(0, 8*DSIZE)

#-------------------------------------------------------------------------------
def pointer_read_stream():
    ...
    while read_size > 0:
        ram.write_nonblocking(read_addr, dma_size if dma_size < read_size else read_size)
