...
# Computation
#-------------------------------------------------------------------------------
mem0 = CoramMemory(idx=0, datawidth=8*DSIZE, size=SIZE, length=4, scattergather=False) # source
mem_d0 = CoramMemory(idx=4, datawidth=8*DSIZE, size=SIZE*2) # destination
...
def st_step(mesh_size, read_start, write_start):
    ...
    mem0.write(SIZE, read_addr, mesh_size)
    read_addr += mesh_size * DSIZE

    mem0.write(SIZE+SIZE, read_addr, mesh_size)
    read_addr += mesh_size * DSIZE
    ...
    for i in range(mesh_size - 2):
        ...
        if read_page == 0:
            ...
            mem0.write_nonblocking(SIZE, read_addr, mesh_size)
        elif read_page == 2:
            mem0.write_nonblocking(SIZE+SIZE, read_addr, mesh_size)
        elif read_page == 3:
            mem0.write_nonblocking(SIZE+SIZE+SIZE, read_addr, mesh_size)
        ...
        if write_page == 0:
            ...
            mem_d0.read_nonblocking(SIZE+1, write_addr, mesh_size-2)
