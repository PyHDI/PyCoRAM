...
#-------------------------------------------------------------------------------
def make_graph(datfile):
    global num_nodes
    global num_edges
    global actual_num_nodes
    global actual_num_edges
    global node_addr
    ...
    for from_node in range(num_nodes):
        for to_node in range(num_nodes):
            ...
            prob = random.random()
            if ((1.0 * num_edges) / (1.0 * num_nodes * num_nodes)) < prob: continue
    ...
    of = open(datfile, 'w')
    of.write('%d %d\n' % (actual_num_nodes, actual_num_edges))
    of.write(tmp)
    of.close()
...
def make_page_list(ofilename):
    ...
    for node_id, node in sorted(nodes.items(), key=lambda x:x[1].my_addr):
        ...
        node.page_addr = page_addr if num_of_edges > 0 else 0
...
if __name__ == '__main__':
    ...
    VERSION = "ver.1.0.0"
    USAGE = "Usage: python memgen.py -o outputfilename [options]"
    ...
    optparser.add_option("-d","--datfile",dest="datfile",
                         default="out.dat",help="Output dat file name, default=out.dat")
    ...
    optparser.add_option("--num_nodes",dest="num_nodes",type=int,
                         default=100,help="Number of Nodes, default=100")
    optparser.add_option("--num_edges",dest="num_edges",type=int,
                         default=10000,help="Number of Edges, default=10000")
    ...
    num_edges = options.num_edges

    make_graph(options.datfile)
    ...
    print("# num of nodes: %d (%d)" % (actual_num_nodes, num_nodes))
    print("# num of edges: %d (%d)" % (actual_num_edges, num_edges))
