...
#-------------------------------------------------------------------------------
mem_a = CoramMemory(idx=0, datawidth=8*DSIZE*SIMD_WIDTH, length=2, size=MM_MEM_A_SIZE, scattergather=False)
...
def mm_computation(matrix_size):
    ...
    for i in range(matrix_size):
        ...
        mem_a.read_nonblocking(MM_MEM_A_SIZE+0, c_addr, matrix_size / SIMD_WIDTH)
...
def mm_main():
    ...
    crc = mm_computation(matrix_size)
